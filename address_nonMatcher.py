...
def normalize_address(raw) -> str:
    # coerce to string, guard against NaN
    s = "" if pd.isna(raw) else str(raw).lower()
    # strip out punctuation, collapse spaces
    s = re.sub(r"[^\w\s]", " ", s)
...
def main(start: int, end: int, output_csv: str):
    ...
    print(f"→ {'Appended' if not write_header else 'Saved'} {len(out_df)} non‑matches with scores in [{LOWER_THRESH},{UPPER_THRESH}) → {output_csv}")
